...
CREATE TABLE node_state (
  ...
  can_load INTEGER NOT NULL CHECK(can_load IN (0, 1)),
  can_unload INTEGER NOT NULL CHECK(can_unload IN (0, 1)),
...
CREATE TABLE slot_state (
  ...
  CHECK((status = 'ready' AND model_id IS NOT NULL) OR (status != 'ready')),
  CHECK((status = 'unloaded' AND model_id IS NULL AND operation_id IS NULL) OR status != 'unloaded'),
  FOREIGN KEY(singleton) REFERENCES control_meta(singleton),
...
CREATE TABLE operations (
  ...
  kind TEXT NOT NULL CHECK(kind IN ('load', 'unload', 'download', 'cancel')),
