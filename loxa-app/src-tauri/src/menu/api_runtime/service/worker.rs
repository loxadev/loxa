...
fn run_failed_worker(requests: Receiver<ServiceRequest>, messages: Sender<ServiceEvent>) {
    if messages.send(ServiceEvent::ControllerFailed).is_err() {
        return;
    }
...
impl ServiceRuntimeWorker {
    fn run(mut self) {
        loop {
            ...
            match self.requests.try_recv() {
                Ok(ServiceRequest::Shutdown { reply }) => {
    ...
    fn handle_request(&mut self, request: ServiceRequest) {
        match request {
            ...
            ServiceRequest::Stop { generation, target } => self.stop(generation, target),
            ServiceRequest::Probe => self.probe(),
    ...
    fn start(&mut self, ui_generation: u64, model_id: String, cancellation: ApiStartCancellation) {
        if self.pending.is_some() {
            self.send_start_result(ui_generation, StartOutcome::Conflict);
            return;
        }
    ...
    fn resolve_start(&mut self, start: PendingStart, observed: &ServiceObservation) {
            ...
            match cleanup_observation(&start.target, observed) {
                ...
                CleanupObservation::Failed(endpoint) => {
                    let endpoint = endpoint.or_else(|| {
                        Some(ApiEndpoint::service(start.model_id, start.target.clone()))
                    });
                    self.send_stopped(Some(start.ui_generation), Err(()), endpoint);
                }
            }
        ...
        match cleanup_observation(&start.target, observed) {
            CleanupObservation::Complete if observed.target().is_some() => {
                self.clear_admitted_target(&start.target);
                self.send_start_result(start.ui_generation, StartOutcome::Conflict);
            }
            CleanupObservation::Complete => {
                self.clear_admitted_target(&start.target);
                self.send_start_result(start.ui_generation, StartOutcome::StartupFailed);
            }
            CleanupObservation::Failed(endpoint) => {
                let endpoint =
                    endpoint.unwrap_or_else(|| ApiEndpoint::service(start.model_id, start.target));
                self.send_start_result(start.ui_generation, StartOutcome::CleanupFailed(endpoint));
            }
            CleanupObservation::Pending => {
    ...
    fn admitted_endpoint(&self, target: &OperationTarget) -> Option<ApiEndpoint> {
        self.admitted_start.as_ref().and_then(|accepted| {
            (&accepted.target == target)
                .then(|| ApiEndpoint::service(accepted.model_id.clone(), accepted.target.clone()))
        })
    }
    ...
    fn send_start_result(&self, generation: u64, outcome: StartOutcome) {
        let _ = self.messages.send(ServiceEvent::Started {
            generation,
            outcome,
        });
    }
    ...
    fn send_stopped(
        ...
        let _ = self.messages.send(ServiceEvent::Stopped {
            generation,
            result,
            endpoint,
        });
...
fn clean_shutdown_reply() -> ServiceShutdownReply {
    ServiceShutdownReply {
        generation: None,
        result: Ok(()),
        endpoint: None,
    }
...
fn classify_start_rejection(error: loxa_ipc::ServiceError) -> StartOutcome {
    start_failure(error.category)
}

fn start_failure(category: ErrorCategory) -> StartOutcome {
    match category {
        ErrorCategory::Busy | ErrorCategory::Conflict => StartOutcome::Conflict,
        ErrorCategory::NotFound | ErrorCategory::ModelUnavailable => StartOutcome::ModelUnavailable,
        _ => StartOutcome::StartupFailed,
    }
}
...
mod tests {
    ...
    use super::{ServiceEvent, ServiceObservationSlot};
    use loxa::api_runtime::ApiStartCancellation;
    use loxa_ipc::{
