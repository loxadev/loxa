...
const operation = {
  id: "op-1", kind: "download", status: "running", model_id: "gemma-3-4b-it-q4",
  progress: { completed_bytes: 5, total_bytes: 10 }, error: null,
...
describe("streamControlEvents", () => {
  it("authenticates, resumes by cursor, and decodes snapshot plus operation frames across byte splits", async () => {
    ...
    await expect(handle.finished).resolves.toEqual({ kind: "error", cursor: 5, message: "Live model updates disconnected." });
  ...
  it("disposal aborts the body and suppresses late callbacks", async () => {
    ...
    const reader = {
      read: vi.fn(() => new Promise<ReadableStreamReadResult<Uint8Array>>((resolve) => {
        releaseRead = () => resolve({ done: false, value: encode(`event: operation\ndata: ${JSON.stringify({ sequence: 2, operation })}\n\n`) });
      })),
      cancel,
      releaseLock: vi.fn(),
    };
  ...
  it("fails closed on malformed or regressing event streams", async () => {
    ...
    await expect(streamControlEvents("http://127.0.0.1:8080", token, 0, observed.value, undefined, fetch).finished).resolves.toMatchObject({ kind: "error", message: expect.stringMatching(/invalid/i) });
  ...
  it.each([
    ...
    const result = await streamControlEvents(
      "http://127.0.0.1:8080",
      token,
      0,
      observed.value,
  ...
  it("resets the buffered-frame budget after each valid frame", async () => {
    ...
    const fetch = vi.fn(async () => responseFrom([
    ...
    const result = await streamControlEvents(
      "http://127.0.0.1:8080", token, 0, observed.value, undefined, fetch,
    ).finished;
  ...
  it("times out a stalled connection and reports a sanitized error", async () => {
    ...
    try {
      ...
      await expect(handle.finished).resolves.toEqual({
        kind: "error", cursor: 0, message: "Connecting to live model updates timed out.",
      });
      expect(observed.terminals).toHaveLength(1);
    } finally {
      vi.useRealTimers();
  ...
  it("rejects an unterminated final control frame instead of silently dropping it", async () => {
    ...
    expect(result).toEqual({
      kind: "error", cursor: 0, message: "The Loxa node returned an invalid model update stream.",
    });
    expect(observed.snapshots).toEqual([]);
  });
});
