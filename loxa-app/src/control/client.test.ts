...
describe("control client", () => {
  ...
  it("uses only known recipe IDs for download and closes operation routes", async () => {
    const fetch = vi
      ...
      .mockResolvedValueOnce(Response.json({
        id: "op-1", kind: "download", status: "queued", model_id: "gemma-3-4b-it-q4",
        progress: null, error: null, created_at_unix_ms: 1, updated_at_unix_ms: 1,
      }))
      .mockResolvedValueOnce(Response.json({
        id: "op-1", kind: "download", status: "cancelled", model_id: "gemma-3-4b-it-q4",
        progress: null, error: null, created_at_unix_ms: 1, updated_at_unix_ms: 2,
      }));

    await expect(downloadModel("http://127.0.0.1:8080", token, "gemma-3-4b-it-q4", { fetch })).resolves.toEqual({ operationId: "op-1" });
    await expect(getOperation("http://127.0.0.1:8080", token, "op-1", { fetch })).resolves.toMatchObject({ status: "queued" });
    await expect(cancelOperation("http://127.0.0.1:8080", token, "op-1", { fetch })).resolves.toMatchObject({ status: "cancelled" });
  ...
  it("decodes capabilities and inventory", async () => {
    const fetch = vi
      .fn()
      .mockResolvedValueOnce(Response.json({ document_input: false, document_input_reason: "Text only", text_chat: true }))
      .mockResolvedValueOnce(Response.json([]));
    await expect(getCapabilities("http://127.0.0.1:8080", token, { fetch })).resolves.toMatchObject({ documentInput: false });
  ...
  it("preserves actionable closed control errors without leaking the token", async () => {
    const fetch = vi.fn(async () => Response.json({ code: "operation_conflict", message: "download active" }, { status: 409 }));
    const error = await downloadModel("http://127.0.0.1:8080", token, "gemma-3-4b-it-q4", { fetch }).catch((reason: unknown) => reason);
  ...
  it.each(["", "wrong", "AB".repeat(32)])("rejects an unsafe token before fetch: %j", async (unsafeToken) => {
    const fetch = vi.fn();
    await expect(getControlNode("http://127.0.0.1:8080", unsafeToken, { fetch })).rejects.toMatchObject({ kind: "credential" });
    expect(fetch).not.toHaveBeenCalled();
  });
  ...
  it("distinguishes timeout, caller cancellation, transport, and malformed payloads", async () => {
    ...
    await expect(getControlNode("http://127.0.0.1:8080", token, { fetch: timeoutFetch, timeoutMs: 1 })).rejects.toMatchObject({ kind: "timeout" });

    const caller = new AbortController();
    caller.abort();
    await expect(getControlNode("http://127.0.0.1:8080", token, { signal: caller.signal })).rejects.toMatchObject({ kind: "aborted" });
    await expect(getControlNode("http://127.0.0.1:8080", token, { fetch: vi.fn(async () => { throw new Error("secret network detail"); }) })).rejects.toMatchObject({ kind: "transport" });
    await expect(getControlNode("http://127.0.0.1:8080", token, { fetch: vi.fn(async () => Response.json({ nope: true })) })).rejects.toMatchObject({ kind: "invalid-response" });
  });
  ...
  it("stops reading and cancels a control response as soon as its byte limit is exceeded", async () => {
    ...
    await expect(getControlNode("http://127.0.0.1:8080", token, {
