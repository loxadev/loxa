...
describe("packaged brand asset", () => {
  it("emits the Loxa mark as a bundled file under strict img-src CSP", () => {
    ...
    const transcriptSource = readFileSync(`${process.cwd()}/src/chat/ChatTranscript.tsx`, "utf8");
    ...
    expect(transcriptSource).toContain('import mark from "../assets/brand/loxa-mark.svg?no-inline"');
    expect(transcriptSource).not.toContain('import mark from "../assets/brand/loxa-mark.svg";');
  });
});
